...
Do the steps in order for input: $ARGUMENTS
...
IN: the input text.
Do not use the Agent tool or any subagent (including techtest-echo) for this step.
...
IN: the input text.
Use the Agent tool to invoke the `techtest-echo` subagent with the following prompt
